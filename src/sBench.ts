...
export function sbench(framework: ReactiveFramework) {
  ...
  function createComputation1(
    s1: Reader,
    deps: Array<Signal<any> | Computed<any>>
  ) {
    framework.computed(() => s1(), deps);
  }
  function createComputation2(
    s1: Reader,
    s2: Reader,
    deps: Array<Signal<any> | Computed<any>>
  ) {
    framework.computed(() => s1() + s2(), deps);
  }
  ...
  function createComputation4(
    ...
    framework.computed(() => s1() + s2() + s3() + s4(), deps);
  }
  ...
  function updateComputations2to1(n: number, sources: Signal<number>[]) {
    let { read: get1, write: set1 } = sources[0],
      { read: get2 } = sources[1];
    framework.computed(() => get1() + get2(), [sources[0], sources[1]]);
    for (let i = 0; i < n; i++) {
      set1(i);
    }
  }
